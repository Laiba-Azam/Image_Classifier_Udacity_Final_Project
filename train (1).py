...
def parse():
    ...
    parser.add_argument('--arch', type=str, default='vgg16', help='model to train')
...
def architecture(m):
    ...
    classifier = nn.Sequential(nn.Linear(in_features=in_features_of_pretrained_model, out_features=500, bias=True),
                           nn.ReLU(inplace=True),
                           nn.Dropout(p=0.2),
                           nn.Linear(in_features=500, out_features=200, bias=True),
                           nn.ReLU(inplace=True),
                           nn.Linear(in_features=200,out_features= 102),
                           nn.LogSoftmax(dim=1)
                          )
    if m.name=="resnet18":
        m.fc = classifier
    ...
    for e in range(epoch):
        ...
        for images, labels in train_loader:
            ...
            if step % print_every == 0 or step == 1 or step == len(train_loader):
                print("Epoch: {}/{} Batch % Complete: {:.2f}%".format(e+1, epoch, (step)*100/len(train_loader))) 
        model.eval()
        with torch.no_grad():
            running_accuracy = 0
            running_valid_loss = 0
