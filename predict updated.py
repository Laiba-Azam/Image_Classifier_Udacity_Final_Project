...
def loading_the_checkpoint(path,gpu):
    #load the saved file
...
def main():
    ...
    args=parse()
    l=loading_the_checkpoint(args.model_checkpoint,args.gpu)
    with open(args.category_name, 'r') as f:
        cat_to_name = json.load(f)
    probs, classes = predict(args.predict_image_path,l,args.topk,cat_to_name)
    for i in range(args.topk):
        print("{} most probable class , Probability {:2f}% ,Class name {}".format(i+1,probs[i]*100,classes[i]))
